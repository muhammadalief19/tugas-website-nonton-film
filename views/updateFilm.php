...
if(isset($_POST["submit"])) {
  $result = $FilmController->update($_POST, $_GET["id"]);
  // var_dump($_POST);
...
$values = $FilmController->edit($_GET["id"]);
?>
<!DOCTYPE html>
...
<div class="navbar bg-base-200 py-6" style="z-index: 10;">
  ...
  <div class="modal-box w-max">
    ...
    <p class="py-4">Selamat Film Berhasil Diupdate !</p>
  ...
  <div class="modal-box w-max">
    ...
    <p class="py-4">Danger, Film gagal diupdate !</p>
    <div class="modal-action">
          ...
          <div class="w-full h-auto flex flex-col gap-10 justify-center items-center">
            ...
            <form action="" method="post" class="w-full h-auto flex flex-col items-center justify-center gap-6">
                <div class="flex flex-col w-4/5 gap-4">
                    ...
                    <input type="text" name="judul" id="judul" class="input input-bordered input-secondary w-full" autocomplete="off" value="<?= $values["judul"] ?>">
                </div>
                <div class="flex flex-col w-4/5 gap-4">
                    ...
                    <select class="select select-secondary w-full text-secondary-focus" name="id_genre">
                        ...
                        <option value="<?= $genre["id_genre"] ?>" <?= ($values["id_genre"] === $genre["id_genre"] ? "selected" : "") ?>><?= $genre["genre"] ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="flex flex-col w-4/5 gap-4">
                    ...
                    <select class="select select-secondary w-full text-secondary-focus" name="id_produser">
                        ...
                        <option value="<?= $produser["id_produser"] ?>" <?= ($values["id_produser"] === $produser["id_produser"] ? "selected" : "") ?> ><?= $produser["nama_produser"] ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="flex flex-col w-4/5 gap-4">
                    <label for="tahun_rilis" class="font-semibold text-secondary-focus">Tahun Rilis</label>
                    <input type="date" name="tahun_rilis" id="tahun_rilis" class="input input-bordered input-secondary w-full text-secondary-focus" autocomplete="off" value="<?= $values["tahun_rilis"] ?>">
                </div>
                <div class="flex flex-col w-4/5 gap-4">
                    <label for="rating" class="font-semibold text-secondary-focus">Rating</label>
                    <input type="text" name="rating" id="rating" class="input input-bordered input-secondary w-full" autocomplete="off" value="<?= $values["rating"] ?>">
                </div>
                <!-- <div class="flex flex-col w-4/5 gap-4">
